...
class Block(abc.ABC):
    ...
    def use_port(self, name: str, original: var) -> var:
        """
        Repurpose `original` port for use in this block. Call it `name`.
        This method is designed to allow subclasses which contain other Block
        instances to expose the ports of those instances at the top level.
        """
...
class AutoCompoundBlock(Block):
    ...
    def define(self, blocks: Set[Block], connectivity: Connectivity) -> None:
        """
        ...
        while len(uncompounded_models) > 1 and iter_num < 3:
            ...
            for t in {type(mu) for mu in uncompounded_models}:
                models_to_compound = {m for m in uncompounded_models if
                                      type(m) == t}
                printv("Compound", [model_str(m) for m in models_to_compound])
                try:
                    compound = t.compound(
                        'auto compound same ' + str(compound_num), block=self,
            ...
            combos = {c for n in range(1, len(uncompounded_models)) for c in
                      combinations(uncompounded_models, n)}
            printv("Finding compounds in combinations",
                   [model_str(m) for models in combos for m in models])
            for models_to_compound in combos:
                try:
                    printv("Trying to compound combo",
                           [model_str(m) for m in models_to_compound])
                    compound = t.compound(
                        'auto compound diff ' + str(compound_num), block=self,
                        models=models_to_compound, connectivity=connectivity)
                    compound_num += 1
                    ...
                    print("AttributeError caught when combining models",
                          models_to_compound)
            printv("Finished round with {:} model(s)".format(
                len(uncompounded_models)))
